...
This package is a bastardized version of @defasm/cli. It removes all of the CLI and file manipulation stuff. All that's left is a way to assemble into ELF executables or object files, suitable for browsers and otherwise. The two functions are `createExecutable` and `createObject`. Their sole parameter is an `AssemblyState` as obtained from @defasm/core. The return value is a `Uint8Array` with an extended prototype.
